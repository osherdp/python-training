...
def write(tcp_client_a):
    ...
    while not exit_flag:
        if msg == "exit":
            exit_flag = True
        tcp_client_a.send(bytes(msg, "utf-8"))
        msg = input("")


def read(tcp_client_a, buffer_size):
    """
    ...
    """
    while True:
        data = tcp_client_a.recv(buffer_size).decode("utf-8")
        print(data)
...
def main():
    ...
    # Write thread.
    w_thread = Thread(target=write, args=(tcp_client_a,))

    # Read thread.
    r_thread = Thread(target=read, args=(tcp_client_a, buffer_size))
